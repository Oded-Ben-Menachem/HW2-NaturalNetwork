...
class MomentumSGD(Optimizer):
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            # 1. Isolation: Compute gradient safely in a new tensor without updating dp in-place
            grad = dp + self.reg * p
            
            # 2. Accumulate velocity in the POSITIVE direction of the gradient: v = mu * v + grad
            self.velocities[p] = self.momentum * self.velocities[p] + grad
            
            # 3. Update weights by SUBTRACTING the accumulated velocity: p = p - lr * v
            # (Just like your working VanillaSGD: p -= lr * dp)
            p -= self.learn_rate * self.velocities[p]
            # ========================
...
class RMSProp(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, decay=0.99, eps=1e-8):
        ...
        self.r_dict = {}
        for p, dp in self.params:
            self.r_dict[p] = torch.zeros_like(p)
        # ========================
    ...
    def step(self):
        for i, (p, dp) in enumerate(self.params):
            ...
            # parameters tensor.
            # ====== YOUR CODE: ======
            # 1. Compute actual gradient safely in a new tensor with L2 regularization
            grad = dp + self.reg * p
            
            # 2. Fetch the current running average for this parameter
            r = self.r_dict[p]
            
            # 3. Update the running average from formula: r = gamma * r + (1 - gamma) * (grad ^ 2)

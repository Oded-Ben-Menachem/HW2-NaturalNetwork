...
class LeakyReLU(Layer):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        out = torch.where(x>=0,x,x*self.alpha)
        # ========================
...
class Linear(Layer):
    ...
    def __init__(self, in_features, out_features, wstd=0.1):
        ...
        # ====== YOUR CODE: ======
        self.w = wstd*torch.randn(self.out_features, self.in_features)
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        loss_vector = -correct_class_scores + log_sum_exp
    ...
    def backward(self, dout=1.0):
        ...
        # ====== YOUR CODE: ======
        probabilities = torch.exp(x) / torch.sum(torch.exp(x), dim=1, keepdim=True)
        probabilities[torch.arange(N), y] -= 1.0
        dx = (probabilities / N) * dout
        # ========================
